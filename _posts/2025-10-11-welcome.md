...
- The sum across all classes is 1:  
  ...
  \]
- Softmax preserves the **order** of scores: if \(z_i > z_j\), then \(\text{softmax}(z)_i > \text{softmax}(z)_j\).  
- Softmax is **invariant to constant shifts**: adding the same constant \(c\) to all logits doesn’t change the output:
  \[
  \text{softmax}(\mathbf{z} + c) = \text{softmax}(\mathbf{z})
  \]  
  This helps with numerical stability. :contentReference[oaicite:2]{index=2}
...
This avoids overflow from large exponentials. :contentReference[oaicite:3]{index=3}
