...
TPUs are best understood as a **distributed machine built out of predictable, structured communication layers**. The hierarchy—core, chip, tray, rack, pod slice, pod, superpod—exists because physics, serviceability, and scale force it. The brilliance is that the software stack presents these layers as something you can program against, rather than something you constantly fight.

In later posts, we can go deeper into how real training parallelism maps to these layers: data-parallel collectives on ICI, model sharding across mesh dimensions, pipeline schedules, and how serving systems exploit pod slices efficiently.
